...
//    }
   ...
   int maxDensity = 0;
   Point* maxDensityPoint = NULL;
   for(double i = minX; i <= maxX; i+=0.1){
      for(double j = minY; j <= maxY; j+=0.1){
    // for(double i = minX; i <= maxX; i++){
    //   for(double j = minY; j <= maxY; j++){
         Point* p = ptNew(i,j);
         int density = listSize(pdctBallSearch(pd, p,0.1));
         if(density > maxDensity){
               maxDensity = density;
               maxDensityPoint = p;
               printf("New max density: %d at (%f,%f)\n", maxDensity, ptGetx(maxDensityPoint), ptGety(maxDensityPoint));
         }
      }
    }
    //Transorm maxDensityPoint to longitude and latitude
    Point* maxDensityPointLL = transformToLL(ptGetx(maxDensityPoint), ptGety(maxDensityPoint));
    printf("Max density: %d at (%f,%f)\n", maxDensity, ptGetx(maxDensityPointLL), ptGety(maxDensityPointLL));
